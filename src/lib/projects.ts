export type Platform = "youtube" | "instagram" | "gdrive";

export type Project = {
  id: string;
  title: string;
  description: string;
  duration: string;
  platform: Platform;
  url: string;
  thumbnail: string;
  aspect?: "16:9" | "9:16";
};

export type ProjectGroup = {
  id: string;
  title: string;
  blurb?: string;
  layout: "feature" | "wide" | "reels";
  projects: Project[];
};

const ytId = (url: string) => {
  const m = url.match(/(?:v=|youtu\.be\/)([\w-]+)/);
  return m ? m[1] : "";
};

const driveId = (url: string) => {
  const m = url.match(/\/d\/([\w-]+)/);
  return m ? m[1] : "";
};

export const embedUrl = (p: Project): string | null => {
  switch (p.platform) {
    case "youtube":
      return `https://www.youtube.com/embed/${ytId(p.url)}?rel=0`;
    case "gdrive":
      return `https://drive.google.com/file/d/${driveId(p.url)}/preview`;
    case "instagram":
      return null;
  }
};

export const showreel: Project = {
  id: "showreel",
  title: "Animation Showreel",
  description:
    "A short compilation showcasing my best 2D and After Effects animation work.",
  duration: "00:52",
  platform: "youtube",
  url: "https://youtu.be/paxp9HqFeH8",
  thumbnail: "/thumbnails/scared-to-death.jpg",
  aspect: "16:9",
};

export const groups: ProjectGroup[] = [
  {
    id: "production",
    title: "Video Production & Motion",
    blurb:
      "Educational and commercial work — from scripting and animation to live-action editing.",
    layout: "wide",
    projects: [
      {
        id: "webseries-intro",
        title: "Mini Web Series — Intro",
        description: "Motion design intro I created for a mini web series.",
        duration: "00:28",
        platform: "youtube",
        url: "https://www.youtube.com/watch?v=Qu-WFRHznvg",
        thumbnail: "/thumbnails/webseries-intro.jpg",
        aspect: "16:9",
      },
      {
        id: "malar-allergic",
        title: "Malar — First Aid Training",
        description:
          "After Effects animation. One of the educational videos I made for the Malar First Aid Training Center.",
        duration: "03:53",
        platform: "youtube",
        url: "https://www.youtube.com/watch?v=bWp91vRg2Xg",
        thumbnail: "/thumbnails/malar-allergic.jpg",
        aspect: "16:9",
      },
      {
        id: "hebrew-u-lesson",
        title: "Hebrew University Lesson",
        description:
          "After Effects editing with pre-made templates. One of many lessons I edited for the Hebrew University.",
        duration: "19:20",
        platform: "gdrive",
        url: "https://drive.google.com/file/d/1JsoCqzg3gWxdw3QambZAYFMgHmK2yG5C/view",
        thumbnail: "/thumbnails/hebrew-u-lesson.jpg",
        aspect: "16:9",
      },
      {
        id: "idf-medical",
        title: "IDF Medical School",
        description:
          "Camera assistant and additional editing on educational videos for the medical school in the Israeli Defense Force.",
        duration: "03:14",
        platform: "youtube",
        url: "https://www.youtube.com/watch?v=fE7iGSEdygU",
        thumbnail: "/thumbnails/idf-medical.jpg",
        aspect: "16:9",
      },
    ],
  },
  {
    id: "social",
    title: "Online Platforms",
    blurb:
      "Reels and short-form content for social media. Reserve duty work alongside Noa Cochva, plus brand work.",
    layout: "reels",
    projects: [
      {
        id: "cochva-1",
        title: "Israel Map — Reel",
        description:
          "Produced during reserve duty after October 7th alongside Noa Cochva. Production assistance and additional editing.",
        duration: "01:20",
        platform: "instagram",
        url: "https://www.instagram.com/reel/Cy_LWywo4SL/",
        thumbnail: "/thumbnails/cochva-israel-map.jpg",
        aspect: "9:16",
      },
      {
        id: "cochva-2",
        title: "Israel — Reel",
        description:
          "Informational reel about the situation in Israel, with Noa Cochva. Production assistance and additional editing.",
        duration: "01:20",
        platform: "instagram",
        url: "https://www.instagram.com/reel/C0raCZ9oAuj/",
        thumbnail: "/thumbnails/cochva-hamas.jpg",
        aspect: "9:16",
      },
      {
        id: "original-commercial",
        title: "Original — Commercial",
        description: "A commercial I created for Original's social platforms.",
        duration: "00:23",
        platform: "instagram",
        url: "https://www.instagram.com/reel/DHnwtpds4fd/",
        thumbnail: "/thumbnails/original-cafe.jpg",
        aspect: "9:16",
      },
    ],
  },
  {
    id: "self",
    title: "Videos Featuring Me",
    blurb:
      "Personal and experimental work — short and long-form formats, in front of and behind the camera.",
    layout: "wide",
    projects: [
      {
        id: "short-vanitas",
        title: "Short-Form Test — Vanitas",
        description:
          "Exploring the short-form video format. Art history visual analysis.",
        duration: "01:00",
        platform: "gdrive",
        url: "https://drive.google.com/file/d/1Fn0QODsyCVlYlvgFfwts_UcL0DktZ5Ei/view",
        thumbnail: "/thumbnails/short-vanitas.jpg",
        aspect: "9:16",
      },
      {
        id: "short-happiness",
        title: "Short-Form Test — Happiness",
        description:
          "Exploring the short-form video format. How to buy happiness, according to research.",
        duration: "01:00",
        platform: "gdrive",
        url: "https://drive.google.com/file/d/1do9C1WlRGYuvJBLQekVErKIHJajklPAI/view",
        thumbnail: "/thumbnails/short-happiness.jpg",
        aspect: "9:16",
      },
      {
        id: "short-2025",
        title: "Short-Form Test — 2025",
        description:
          "Exploring the short-form video format. Reflective piece on memory and the new year.",
        duration: "01:00",
        platform: "gdrive",
        url: "https://drive.google.com/file/d/19KiQZ8hYauTmRAW19M4de6FJUO2SsFOS/view",
        thumbnail: "/thumbnails/short-2025.jpg",
        aspect: "9:16",
      },
      {
        id: "bezalel-doc",
        title: "Short Self-Documentary",
        description:
          "A short self-documentary I made during my studies at Bezalel.",
        duration: "05:40",
        platform: "gdrive",
        url: "https://drive.google.com/file/d/1vJMmwAlZddafqwwhJHixysXrlpUvzTel/view",
        thumbnail: "/thumbnails/bezalel-doc.jpg",
        aspect: "16:9",
      },
      {
        id: "long-coding",
        title: "Long-Form YouTube Test",
        description:
          "Exploring the long-form video format for YouTube. Personal video essay.",
        duration: "09:27",
        platform: "gdrive",
        url: "https://drive.google.com/file/d/1iqpdzFijwLfKjS46FSifOdkURM-ZKRdG/view",
        thumbnail: "/thumbnails/no-coding-exp.jpg",
        aspect: "16:9",
      },
    ],
  },
];
